...
WORKDIR="${CF7VK_E5_PLUGIN_CHECK_WORKDIR:-$(mktemp -d "${TMPDIR:-/tmp}/cf7vk-e5-plugin-check-${RUN_ID}.XXXXXX")}"
CACHE_DIR="${CF7VK_E5_PLUGIN_CHECK_CACHE_DIR:-${XDG_CACHE_HOME:-${HOME}/.cache}/cf7-vk/e5-plugin-check}"
RESULTS_DIR="${CF7VK_E5_PLUGIN_CHECK_RESULTS_DIR:-${WORKDIR}/results}"
...
WP_VERSION="${CF7VK_E5_PLUGIN_CHECK_WP_VERSION:-$(jq -r '.wordpress.default_core_version' "${SOURCE_MANIFEST}")}"
WP_CLI_IMAGE="${CF7VK_E5_PLUGIN_CHECK_WP_CLI_IMAGE:-$(jq -r '.wordpress.default_cli_image' "${SOURCE_MANIFEST}")}"
CF7_VERSION="${CF7VK_E5_PLUGIN_CHECK_CF7_VERSION:-$(jq -r '.dependencies.contact_form_7.default_version' "${SOURCE_MANIFEST}")}"
PLUGIN_CHECK_VERSION="${CF7VK_PLUGIN_CHECK_VERSION:-1.9.0}"
PLUGIN_CHECK_SHA256="${CF7VK_PLUGIN_CHECK_SHA256:-028330721e01041a28d1465e2c802357119870580670f2dcf00326af859da9a2}"
PLUGIN_CHECK_URL="${CF7VK_PLUGIN_CHECK_URL:-https://downloads.wordpress.org/plugin/plugin-check.${PLUGIN_CHECK_VERSION}.zip}"
CANDIDATE_ZIP="${CF7VK_CANDIDATE_ZIP:-${REPO_ROOT}/dist/${PLUGIN_SLUG}-wp-plugin.zip}"
EXPECTED_CANDIDATE_VERSION="${CF7VK_EXPECTED_CANDIDATE_VERSION:-}"
KEEP_WORKDIR=0
FAILURES=0
SUMMARY_WRITTEN=0
CURRENT_PROJECT=""
...
Environment:
  ...
  CF7VK_EXPECTED_CANDIDATE_VERSION       Expected candidate version.
  CF7VK_PLUGIN_CHECK_VERSION             Pinned Plugin Check version. Default 1.9.0.
  CF7VK_PLUGIN_CHECK_SHA256              Expected Plugin Check ZIP SHA-256.
  CF7VK_E5_PLUGIN_CHECK_WP_VERSION       WordPress core version.
  CF7VK_E5_PLUGIN_CHECK_WP_CLI_IMAGE     WP-CLI Docker image.
  CF7VK_E5_PLUGIN_CHECK_CF7_VERSION      Contact Form 7 version.
  CF7VK_E5_PLUGIN_CHECK_RESULTS_DIR      Evidence output directory.
USAGE
}
...
source_header_version() {
	sed -nE 's/^[[:space:]]*\*[[:space:]]*Version:[[:space:]]*([^[:space:]]+).*/\1/p' "${REPO_ROOT}/plugin-dir/${ENTRYPOINT}" | head -n 1
}
...
prepare_candidate() {
	...
	if [ -z "${expected_version}" ]; then
		fail_step "candidate" "expected_version" "Expected candidate version is not available." "$(jq -nc --arg file "${REPO_ROOT}/plugin-dir/${ENTRYPOINT}" '{source:$file}')"
		exit 2
	fi
...
setup_site() {
	...
	run_logged "wordpress" "core_install" wp_run core install --path=/var/www/html --url="http://${CURRENT_PROJECT}.test" --title="CF7VK Plugin Check" --admin_user=admin --admin_password=admin-password --admin_email=admin@example.test

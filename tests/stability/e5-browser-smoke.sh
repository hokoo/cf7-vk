...
WORKDIR="${CF7VK_E5_BROWSER_WORKDIR:-$(mktemp -d "${TMPDIR:-/tmp}/cf7vk-e5-browser-${RUN_ID}.XXXXXX")}"
RESULTS_DIR="${CF7VK_E5_BROWSER_RESULTS_DIR:-${WORKDIR}/results}"
...
WP_VERSION="${CF7VK_E5_BROWSER_WP_VERSION:-$(jq -r '.wordpress_version // .wordpress.default_core_version // "7.1"' <<<"${CURRENT_ROW}")}"
...
IMAGE_PROBE_TIMEOUT="${CF7VK_E5_BROWSER_IMAGE_PROBE_TIMEOUT:-90s}"
INSTALL_BROWSER="${CF7VK_E5_BROWSER_INSTALL_BROWSER:-1}"
KEEP_WORKDIR=0
FAILURES=0
SUMMARY_WRITTEN=0
CURRENT_PROJECT=""
...
Environment:
  ...
  CF7VK_E5_BROWSER_WP_WEB_IMAGE       Apache WordPress image. Defaults to versioned image if available, else generic php image.
  CF7VK_E5_BROWSER_WEB_PORT           Host port. Defaults to a random high port.
USAGE
}
...
resolve_web_image() {
	...
	if [ -n "${CF7VK_E5_BROWSER_WP_WEB_IMAGE:-}" ]; then
		WP_WEB_IMAGE="${CF7VK_E5_BROWSER_WP_WEB_IMAGE}"
...
setup_site() {
	...
	run_logged "core-install" wp_run core install --path=/var/www/html --url="${BASE_URL}" --title="CF7VK E5 Browser Smoke" --admin_user=admin --admin_password=admin-password --admin_email=admin@example.test || exit 1
...
run_browser_smoke() {
	...
	(
		...
		CF7VK_E5_BROWSER_BASE_URL="${BASE_URL}" \
		CF7VK_E5_BROWSER_RESULTS_DIR="${RESULTS_DIR}" \
		CF7VK_E5_BROWSER_RESULT_JSON="${BROWSER_RESULT_JSON}" \
		CF7VK_E5_BROWSER_PLAYWRIGHT_REPORT_JSON="${PLAYWRIGHT_REPORT_JSON}" \
		CF7VK_EXPECTED_CANDIDATE_VERSION="${EXPECTED_CANDIDATE_VERSION}" \
		CF7VK_CANDIDATE_SHA256="$(candidate_sha256)" \
		CF7VK_E5_BROWSER_EXPECTED_WP_VERSION="${WP_VERSION}" \

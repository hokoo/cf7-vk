...
WORKDIR="${CF7VK_E4_SMOKE_WORKDIR:-$(mktemp -d "${TMPDIR:-/tmp}/cf7vk-e4-smoke-${RUN_ID}.XXXXXX")}"
RESULTS_DIR="${CF7VK_E4_SMOKE_RESULTS_DIR:-${WORKDIR}/results}"
...
WP_VERSION="${CF7VK_E4_SMOKE_WP_VERSION:-$(jq -r '.wordpress.default_core_version' "${SOURCE_MANIFEST}")}"
WP_CLI_IMAGE="${CF7VK_E4_SMOKE_WP_CLI_IMAGE:-$(jq -r '.wordpress.default_cli_image' "${SOURCE_MANIFEST}")}"
CF7_VERSION="${CF7VK_E4_SMOKE_CF7_VERSION:-$(jq -r '.dependencies.contact_form_7.default_version' "${SOURCE_MANIFEST}")}"
...
Environment:
  ...
  CF7VK_E4_SMOKE_WP_VERSION         WordPress core version.
  CF7VK_E4_SMOKE_WP_CLI_IMAGE       WP-CLI Docker image.
  CF7VK_E4_SMOKE_CF7_VERSION        Contact Form 7 version.
  CF7VK_E4_SMOKE_RESULTS_DIR        Evidence output directory.
USAGE
}

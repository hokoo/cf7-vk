...
WORKDIR="${CF7VK_E5_SUPPORT_MATRIX_WORKDIR:-$(mktemp -d "${TMPDIR:-/tmp}/cf7vk-e5-support-matrix-${RUN_ID}.XXXXXX")}"
RESULTS_DIR="${CF7VK_E5_SUPPORT_MATRIX_RESULTS_DIR:-${WORKDIR}/results}"
...
EXPECTED_CANDIDATE_VERSION="${CF7VK_EXPECTED_CANDIDATE_VERSION:-$(jq -r '.candidate.expected_version // .support_contract.candidate_expected_version // empty' "${SOURCE_MANIFEST}")}"
PROMOTION_ROLLBACK_VERSION="${CF7VK_E5_PROMOTION_ROLLBACK_VERSION:-$(jq -r '.release_assumptions.wordpress_org_production_baseline // empty' "${SOURCE_MANIFEST}")}"
IMAGE_PROBE_TIMEOUT="${CF7VK_E5_SUPPORT_MATRIX_IMAGE_PROBE_TIMEOUT:-90s}"
KEEP_WORKDIR=0
FAILURES=0
SUMMARY_WRITTEN=0
ROWS=()
...
Environment:
  ...
  CF7VK_EXPECTED_CANDIDATE_VERSION            Expected candidate version, default from manifest candidate.expected_version.
  CF7VK_E5_SUPPORT_MATRIX_IMAGE_PROBE_TIMEOUT Docker manifest probe timeout, default 90s.
  CF7VK_E5_SUPPORT_MATRIX_RESULTS_DIR         Evidence output directory.
USAGE
}
...
run_row() {
	...
	set +e
	CF7VK_CANDIDATE_ZIP="${CANDIDATE_ZIP}" \
	CF7VK_EXPECTED_CANDIDATE_VERSION="${EXPECTED_CANDIDATE_VERSION}" \
	CF7VK_E1_WP_VERSION="${wp_version}" \
	CF7VK_E1_WP_CLI_IMAGE="${image}" \
	CF7VK_E1_CF7_VERSION="${cf7_version}" \
	CF7VK_E1_RESULTS_DIR="${row_results}" \
	CF7VK_E1_WORKDIR="${row_workdir}" \
	CF7VK_E1_SUPPORT_ROW_ID="${matrix_id}" \
	CF7VK_E1_SUPPORT_ROW_LABEL="${label}" \
	CF7VK_E1_SUPPORT_ROW_REQUIRED="${required}" \
	CF7VK_E1_SUPPORT_ROW_PHP="${php_version}" \

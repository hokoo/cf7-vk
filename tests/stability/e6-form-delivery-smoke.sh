...
PLUGIN_SLUG="${PLUGIN_SLUG:-$(jq -r '.plugin_slug // "message-bridge-for-contact-form-7-and-vk"' "${SOURCE_MANIFEST}")}"
ENTRYPOINT="${ENTRYPOINT:-$(jq -r '.entrypoint // "cf7-vk.php"' "${SOURCE_MANIFEST}")}"
RUN_ID="$(date -u +%Y%m%dT%H%M%SZ)-$$"
WORKDIR="${CF7VK_E6_WORKDIR:-$(mktemp -d "${TMPDIR:-/tmp}/cf7vk-e6-delivery-${RUN_ID}.XXXXXX")}"
RESULTS_DIR="${CF7VK_E6_RESULTS_DIR:-${WORKDIR}/results}"
LOG_DIR="${RESULTS_DIR}/logs"
ARTIFACT_DIR="${WORKDIR}/artifacts"
RUNTIME_DIR="${WORKDIR}/runtime"
COMPOSE_FILE="${WORKDIR}/docker-compose.e6-delivery.yml"
...
WP_VERSION="${CF7VK_E6_WP_VERSION:-$(jq -r '.wordpress_version // .wordpress.default_core_version // "7.1"' <<<"${CURRENT_ROW}")}"
PHP_VERSION="${CF7VK_E6_PHP_VERSION:-$(jq -r '.php_version // "8.3"' <<<"${CURRENT_ROW}")}"
WP_CLI_IMAGE="${CF7VK_E6_WP_CLI_IMAGE:-$(jq -r '.wp_cli_image // .wordpress.default_cli_image // "wordpress:cli-php8.3"' <<<"${CURRENT_ROW}")}"
CF7_VERSION="${CF7VK_E6_CF7_VERSION:-$(jq -r '.contact_form_7_version // .dependencies.contact_form_7.default_version // "6.1.7"' <<<"${CURRENT_ROW}")}"
...
WEB_PORT="${CF7VK_E6_WEB_PORT:-$(shuf -i 20000-45000 -n 1)}"
BASE_URL="${CF7VK_E6_BASE_URL:-http://127.0.0.1:${WEB_PORT}}"
IMAGE_PROBE_TIMEOUT="${CF7VK_E6_IMAGE_PROBE_TIMEOUT:-90s}"
INSTALL_BROWSER="${CF7VK_E6_INSTALL_BROWSER:-1}"
KEEP_WORKDIR=0
FAILURES=0
SUMMARY_WRITTEN=0
CURRENT_PROJECT=""
...
REQUIRED_CHECKS='["fake-transport-active","public-form-renders","cf7-submit-success","send-message-attempts","no-unexpected-recipient","no-token-leakage","no-page-errors","no-console-errors","partial-failure-cf7-success","partial-failure-recipient-continuity","partial-failure-evidence-redacted","partial-failure-no-page-errors","partial-failure-no-console-errors","admin-empty-state","admin-bot-created","admin-bot-token-validated","admin-channel-created","admin-form-selectable","admin-chat-discovered","admin-relations-assigned","admin-delivery-targets-assigned-chat","admin-evidence-redacted","admin-deletion-safety","admin-no-page-errors","admin-no-console-errors"]'

usage() {
	cat <<'USAGE'
Usage: tests/stability/e6-form-delivery-smoke.sh [options]
...
Environment:
  ...
  CF7VK_E6_RESULTS_DIR             Summary, logs, traces, screenshots, and Playwright report directory.
  CF7VK_E6_WP_VERSION              WordPress core version, default current support row.
  CF7VK_E6_WP_CLI_IMAGE            WP-CLI image, default current support row.
  CF7VK_E6_WP_WEB_IMAGE            Apache WordPress image. Defaults to versioned image if available.
  CF7VK_E6_WEB_PORT                Host port. Defaults to a random high port.
USAGE
}
...
candidate_header_version() {
	unzip -p "$1" "${PLUGIN_SLUG}/${ENTRYPOINT}" | sed -nE 's/^[[:space:]]*\*[[:space:]]*Version:[[:space:]]*([^[:space:]]+).*/\1/p' | head -n 1
}
...
on_exit() {
	...
	if [ -f "${SUMMARY_JSON}" ]; then
		echo "E6 form delivery smoke evidence: ${SUMMARY_JSON}"
	fi
...
resolve_web_image() {
	...
	if [ -n "${CF7VK_E6_WP_WEB_IMAGE:-}" ]; then
		WP_WEB_IMAGE="${CF7VK_E6_WP_WEB_IMAGE}"
...
prepare_candidate() {
	...
	emit "step" "candidate-ready" "pass" "Candidate ZIP staged for E6 form delivery smoke." "$(jq -nc --arg file "${staged_zip}" --arg version "${actual_version}" --arg sha256 "$(candidate_sha256)" '{file:$file,version:$version,sha256:$sha256}')"
}
...
services:
  ...
  wordpress:
    ...
    depends_on:
      - db
    extra_hosts:
      - "api.vk.com:127.0.0.1"
      - "vk.com:127.0.0.1"
      - "lp.vk.example:127.0.0.1"
    ports:
  ...
  cli:
    ...
    volumes:
      ...
      - ${RUNTIME_DIR}:/runtime
      - ${SCRIPT_DIR}:/e6-tests:ro

volumes:
  wp-data:
COMPOSE

	emit "step" "compose-file" "pass" "Wrote isolated E6 Docker Compose file." "$(jq -nc --arg file "${COMPOSE_FILE}" --arg web_image "${WP_WEB_IMAGE}" --arg cli_image "${WP_CLI_IMAGE}" --arg base_url "${BASE_URL}" '{file:$file,web_image:$web_image,cli_image:$cli_image,base_url:$base_url}')"
}
...
setup_site() {
	...
	run_logged "core-install" wp_run core install --path=/var/www/html --url="${BASE_URL}" --title="CF7VK E6 Form Delivery Smoke" --admin_user=admin --admin_password=admin-password --admin_email=admin@example.test || exit 1
	...
	run_logged "seed-form-delivery-fixture" wp_run eval-file /e6-tests/wp-e6-form-delivery-fixture.php || exit 1
...
run_browser_smoke() {
	...
	(
		...
		CF7VK_E6_BASE_URL="${BASE_URL}" \
		CF7VK_E6_RESULTS_DIR="${RESULTS_DIR}" \
		CF7VK_E6_RESULT_JSON="${BROWSER_RESULT_JSON}" \
		CF7VK_E6_PLAYWRIGHT_REPORT_JSON="${PLAYWRIGHT_REPORT_JSON}" \
		CF7VK_EXPECTED_CANDIDATE_VERSION="${EXPECTED_CANDIDATE_VERSION}" \
		CF7VK_CANDIDATE_SHA256="$(candidate_sha256)" \
		npm exec -- playwright test --config "${REPO_ROOT}/tests/e2e/e6-playwright.config.js"
	) >"${log_file}" 2>&1
	exit_code="$?"
	set -e
...
if [ "$(jq -r '.failed_steps' "${SUMMARY_JSON}")" -gt 0 ]; then
	echo "E6 form delivery smoke failed." >&2
	exit 1
fi

echo "E6 form delivery smoke passed."

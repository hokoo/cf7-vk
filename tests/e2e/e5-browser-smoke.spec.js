...
const resultPath = process.env.CF7VK_E5_BROWSER_RESULT_JSON || path.join(
	process.env.CF7VK_E5_BROWSER_RESULTS_DIR || process.cwd(),
	'browser-result.json'
);
...
const expectedWpVersion = process.env.CF7VK_E5_BROWSER_EXPECTED_WP_VERSION || '';
const adminUser = process.env.CF7VK_E5_BROWSER_ADMIN_USER || 'admin';
const adminPassword = process.env.CF7VK_E5_BROWSER_ADMIN_PASSWORD || 'admin-password';
...
const routeName = (url) => {
	...
	if (/\/wp\/v2\/cf7vk_bot\/?$/.test(pathName)) return 'bots';
	if (/\/wp\/v2\/cf7vk_chat\/?$/.test(pathName)) return 'chats';
	if (/\/wp\/v2\/cf7vk_channel\/?$/.test(pathName)) return 'channels';
	if (/\/contact-form-7\/v1\/contact-forms\/?$/.test(pathName)) return 'forms';
	return '';
};
...
const runControlAction = async (page, baseURL, action) => {
	const response = await page.request.post(`${baseURL}/wp-admin/admin-ajax.php`, {
		form: {
			action: 'cf7vk_e5_browser_control',
			e5_action: action,
		},
	});
...
test('candidate admin browser lifecycle smoke', async ({baseURL, page}) => {
	...
	page.on('response', async (response) => {
		...
		if (url.includes('/wp-content/plugins/message-bridge-for-contact-form-7-and-vk/react/build/static/js/main.js')) {
			evidence.asset_responses.push({type: 'js', status: response.status(), url: sanitizeUrl(url)});
		}
		if (url.includes('/wp-content/plugins/message-bridge-for-contact-form-7-and-vk/react/build/static/css/main.css')) {
	...
	await expectCheck('authenticated-admin-render', 'Authenticated admin page renders before and after candidate reactivation.', async () => {
		...
		const runtime = await page.evaluate(() => ({
			...
			candidate_version: window.cf7vkData?.plugin?.version || '',
			has_nonce: Boolean(window.cf7vkData?.nonce),
		}));
	...
	await expectCheck('system-notices-hidden', 'System WordPress notices are hidden while plugin notices remain visible.', async () => {
		const systemNotice = page.locator('#cf7vk-e5-server-notice');
		const pluginNotice = page.locator('#cf7vk-e5-plugin-notice');
	...
	await expectCheck('full-page-background', 'The plugin background fills the complete WordPress content area.', async () => {
		const backgrounds = await page.evaluate(() => {
			const elements = [
				...
				document.querySelector('#cf7-vk-admin-page'),
			];
	...
	await expectCheck('pagination-beyond-ten', 'Bots, chats, channels, and forms beyond the first ten records are loaded in browser REST data.', async () => {
		...
		await waitForRestState(
			page,
			() => ['bots', 'chats', 'channels', 'forms'].every((name) => evidence.collections[name]?.count > 10),
	...
	await expectCheck('post-mutation-observed', 'A protected bot mutation is observed as POST from the real admin screen.', async () => {
		await waitForRestState(
			...
			() => evidence.rest_requests.some((request) => (
				request.method === 'POST'
				&& /\/wp\/v2\/cf7vk_bot\/\d+\/ping/.test(decodeURIComponent(request.url))
				&& request.has_wp_nonce_header
			)),
			'Expected POST /ping mutation with X-WP-Nonce should be observed.'
		);
		return {
			post_mutations: evidence.rest_requests.filter((request) => request.method === 'POST' && request.url.includes('/cf7vk_bot/')),
	...
	await expect(page.locator('.cf7vk-load-status')).toBeVisible();
	await page.getByRole('button', {name: /Retry failed requests/i}).click();
	await expect(page.locator('.channel-list .entity-container.channel')).toHaveCount(12);
	await expect(page.locator('.cf7vk-load-status')).toBeHidden();

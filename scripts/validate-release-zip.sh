...
PLUGIN_SLUG="${PLUGIN_SLUG:-message-bridge-for-contact-form-7-and-vk}"
ENTRYPOINT="${ENTRYPOINT:-cf7-vk.php}"
AUTOLOADER_SUFFIX="${AUTOLOADER_SUFFIX:-Cf7Vk}"
ZIP_PATH="${1:-}"
...
const_version="$(
	{
		sed -nE "s/^[[:space:]]*const[[:space:]]+CF7VK_VERSION[[:space:]]*=[[:space:]]*'([^']+)'.*/\1/p" "$ENTRYPOINT_PATH"
		sed -nE "s/^[[:space:]]*define\\([[:space:]]*'CF7VK_VERSION'[[:space:]]*,[[:space:]]*'([^']+)'.*/\1/p" "$ENTRYPOINT_PATH"
	} | head -n 1
)"
...
[ -n "$const_version" ] || fail "CF7VK_VERSION is not readable"
[ -n "$stable_tag" ] || fail "readme stable tag is not readable"

if [ "$header_version" != "$const_version" ]; then
	fail "plugin header version ($header_version) differs from CF7VK_VERSION ($const_version)"
fi

...
describe('API collections', () => {
    ...
    beforeEach(() => {
        global.cf7vkData = {
            ...
            routes: {
                ...
                chats: 'https://example.test/wp-json/wp/v2/cf7vk_chat/',
                bots: 'https://example.test/wp-json/wp/v2/cf7vk_bot/',
                channels: 'https://example.test/wp-json/wp/v2/cf7vk_channel/',
                relations: {
                    bot2channel: 'https://example.test/wp-json/wp-connections/v1/client/cf7-vk/relation/bot2channel/',
                    chat2channel: 'https://example.test/wp-json/wp-connections/v1/client/cf7-vk/relation/chat2channel/',
                    form2channel: 'https://example.test/wp-json/wp-connections/v1/client/cf7-vk/relation/form2channel/',
                    bot2chat: 'https://example.test/wp-json/wp-connections/v1/client/cf7-vk/relation/bot2chat/',
                },
            },
        };
    ...
    it('rejects with a structured ApiError when a later page fails', async () => {
        global.fetch
            ...
            .mockResolvedValueOnce(response({
                code: 'rest_forbidden',
                message: 'Server error for ?access_token=vk1.privateTokenValueForTestOnly123456',
                data: {
                    status: 403,
                    accessToken: 'vk1.privateTokenValueForTestOnly123456',
                    retry: {
                        nonce: 'test-nonce',
                    },
                },
    ...
    it('distinguishes transport, HTTP, and invalid response failures safely', async () => {
        global.fetch
            .mockRejectedValueOnce(new Error('connect ECONNREFUSED ?access_token=vk1.privateTokenValueForTestOnly123456'))
        ...
        await expect(fetchBots()).rejects.toMatchObject({
            ...
            status: 0,
            message: 'connect ECONNREFUSED ?access_token=[redacted]',
        });
        await expect(fetchBots()).rejects.toMatchObject({
            category: 'rest_http',
            status: 502,
    ...
    it('redacts sensitive query values from ApiError URLs', async () => {
        global.cf7vkData.routes.bots = 'https://example.test/wp-json/wp/v2/cf7vk_bot/?access_token=vk1.privateTokenValueForTestOnly123456&_wpnonce=test-nonce&key=long-poll-key';
        global.fetch.mockResolvedValueOnce(response({code: 'rest_forbidden', message: 'Forbidden', data: {status: 403}}, 1, false, 403));

        await expect(fetchBots()).rejects.toMatchObject({
            url: '/wp-json/wp/v2/cf7vk_bot/?access_token=%5Bredacted%5D&_wpnonce=%5Bredacted%5D&key=%5Bredacted%5D&order=asc&orderby=id&per_page=100&page=1',
        });
    });

    it('loads every bot and channel page through the WordPress posts collection contract', async () => {
        global.fetch
        ...
        expect(fetchUrl(0).searchParams.get('page')).toBe('1');
        expect(fetchUrl(1).searchParams.get('page')).toBe('2');
        expect(fetchUrl(2).searchParams.get('page')).toBe('1');
        expect(fetchUrl(3).searchParams.get('page')).toBe('2');
    });
    ...
    it('creates an empty bot without sending placeholder credentials for validation', async () => {
        global.fetch.mockResolvedValueOnce(response({id: 10}, 1));

        await expect(apiCreateBot({title: 'Bot Name'})).resolves.toEqual({id: 10});
    ...
    it('uses POST for bot action endpoints that can mutate bot state or diagnostics', async () => {
        ...
        expect(global.fetch.mock.calls[0][0]).toBe('https://example.test/wp-json/wp/v2/cf7vk_bot/10/ping');
        expect(global.fetch.mock.calls[0][1].method).toBe('POST');
        expect(global.fetch.mock.calls[1][0]).toBe('https://example.test/wp-json/wp/v2/cf7vk_bot/10/fetch_updates');
        expect(global.fetch.mock.calls[1][1].method).toBe('POST');
    });

    it('adds force delete params outside rest_route endpoint values', async () => {
        global.cf7vkData.routes.chats = 'https://example.test/index.php?rest_route=/wp/v2/cf7vk_chat/';
        global.cf7vkData.routes.bots = 'https://example.test/index.php?rest_route=/wp/v2/cf7vk_bot/';
        global.cf7vkData.routes.channels = 'https://example.test/index.php?rest_route=/wp/v2/cf7vk_channel/';
        ...
        expect(global.fetch.mock.calls[0][0]).toBe('https://example.test/index.php?rest_route=/wp/v2/cf7vk_chat/8&force=true');
        expect(global.fetch.mock.calls[0][1].method).toBe('DELETE');
        expect(global.fetch.mock.calls[1][0]).toBe('https://example.test/index.php?rest_route=/wp/v2/cf7vk_bot/10&force=true');
        expect(global.fetch.mock.calls[1][1].method).toBe('DELETE');
        expect(global.fetch.mock.calls[2][0]).toBe('https://example.test/index.php?rest_route=/wp/v2/cf7vk_channel/12&force=true');
